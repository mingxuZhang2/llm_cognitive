...
#SBATCH --output=/hpc2hdd/home/mzhang630/data/nature/experiments/logs/emp_dis_%j.out
...
echo "=== Performing Empathy Dissection — All 4 Models ($(date)) ==="

for MODEL_PATH in "$QWEN" "$LLAMA" "$MISTRAL" "$GEMMA"; do
    SHORT=$(basename $(dirname $(dirname "$MODEL_PATH")) | sed 's/models--[^-]*--//')
    echo ""
    echo "--- $SHORT ---"
    python3 -u src/performing_empathy_dissection.py --model_path "$MODEL_PATH" --model_short "$SHORT"
done
