#!/bin/bash
#SBATCH --job-name=funcatlas_multi
...
#SBATCH --mem=128G
#SBATCH --time=4:00:00
#SBATCH --output=/data/user/mzhang630/logs/multi_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/multi_%A_%a.err
#SBATCH --array=0-3

# Multi-function dissociation: 8-way causal ablation across 4 models
# For each of 8 categories, ablate its top-5000 causally selective neurons
# and measure PPL across all 8 categories → 8x8 dissociation matrix
...
mkdir -p ${BASE}/results/multi /data/user/mzhang630/logs

echo "Multi-Function Dissociation: ${MODEL_SHORT}"
