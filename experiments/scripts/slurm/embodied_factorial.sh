...
#SBATCH --output=/hpc2hdd/home/mzhang630/data/nature/experiments/logs/emb_fact_%j.out
...
echo "=== Embodied Factorial — All 4 Models ($(date)) ==="

for MODEL_PATH in "$QWEN" "$LLAMA" "$MISTRAL" "$GEMMA"; do
    SHORT=$(basename $(dirname $(dirname "$MODEL_PATH")) | sed 's/models--[^-]*--//')
    echo ""
    echo "--- $SHORT ---"
    python3 -u src/embodied_factorial_extract.py --model_path "$MODEL_PATH" --model_short "$SHORT"
done
