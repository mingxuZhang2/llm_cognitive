...
#SBATCH --mem=128G
#SBATCH --time=8:00:00
#SBATCH --output=/data/user/mzhang630/logs/accuracy_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/accuracy_%A_%a.err
#SBATCH --array=0-3

# Accuracy-based dissociation: 8-way causal ablation with downstream accuracy metrics
# For each of 8 categories, ablate its top-5000 causally selective neurons
# and measure ACCURACY + PPL across all 8 categories on held-out validation set
#
# Uses separate discovery (attribution) and validation (accuracy) stimuli
# to avoid train-on-test leakage.
#
# Time estimate: ~6-8h per model (generation for exact_match is slower than PPL)
...
mkdir -p ${BASE}/results/accuracy /data/user/mzhang630/logs

echo "============================================================"
echo "Accuracy Dissociation: ${MODEL_SHORT}"
