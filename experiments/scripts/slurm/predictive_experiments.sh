#!/bin/bash
#SBATCH --job-name=funcatlas_predict
...
#SBATCH --mem=128G
#SBATCH --time=4:00:00
#SBATCH --output=/data/user/mzhang630/logs/predict_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/predict_%A_%a.err
#SBATCH --array=0-3

# Predictive experiments: pathway decomposition + steering + instance prediction
...
mkdir -p ${BASE}/results/predictive /data/user/mzhang630/logs

echo "Predictive Experiments: ${MODEL_SHORT}"
...
python predictive_experiments.py \
    ...
    --attribution_path "${BASE}/results/multi/${MODEL_SHORT}_multi_attribution.npz" \
    --stimuli "${BASE}/stimuli/stimuli_pilot.jsonl" \
    --output_dir "${BASE}/results/predictive"
