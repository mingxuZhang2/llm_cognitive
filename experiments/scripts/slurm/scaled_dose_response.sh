#!/bin/bash
#SBATCH --job-name=funcatlas_sdose
...
#SBATCH --mem=128G
#SBATCH --time=6:00:00
#SBATCH --output=/data/user/mzhang630/logs/sdose_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/sdose_%A_%a.err
#SBATCH --array=0-3

# Scaled dose-response: 50/cat stimuli, ablation sizes 500-20000
# Shows monotonic specificity increase with neuron count
...
mkdir -p ${BASE}/results/scaled_dose_response /data/user/mzhang630/logs

echo "Scaled Dose-Response: ${MODEL_SHORT}"
...
python dose_response.py \
    ...
    --output_dir "${BASE}/results/scaled_dose_response" \
