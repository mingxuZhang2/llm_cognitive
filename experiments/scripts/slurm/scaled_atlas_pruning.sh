#!/bin/bash
#SBATCH --job-name=funcatlas_sprune
...
#SBATCH --mem=128G
#SBATCH --time=6:00:00
#SBATCH --output=/data/user/mzhang630/logs/sprune_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/sprune_%A_%a.err
#SBATCH --array=0-3

# Scaled atlas-guided pruning: 50/cat stimuli, scaled attribution
# Compare atlas vs magnitude vs random at 10-50% sparsity
# Uses attribution from scaled_multi run (50/cat)
...
mkdir -p ${BASE}/results/scaled_pruning /data/user/mzhang630/logs

echo "Scaled Atlas-Guided Pruning: ${MODEL_SHORT}"
echo "GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null)"

cd ${SRC}
python atlas_pruning.py \
    ...
    --output_dir "${BASE}/results/scaled_pruning" \
