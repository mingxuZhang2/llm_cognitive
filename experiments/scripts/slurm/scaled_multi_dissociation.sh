#!/bin/bash
#SBATCH --job-name=funcatlas_scaled
...
#SBATCH --mem=128G
#SBATCH --time=6:00:00
#SBATCH --output=/data/user/mzhang630/logs/scaled_multi_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/scaled_multi_%A_%a.err
#SBATCH --array=0-3

# Scaled multi-function dissociation on MEDIUM balanced stimulus set (50/category, 400 total)
# Same 4 models as multi_dissociation_all.sh, but with balanced stimuli
...
OUTDIR="${BASE}/results/scaled_multi"
mkdir -p ${OUTDIR} /data/user/mzhang630/logs

echo "Scaled Multi-Function Dissociation: ${MODEL_SHORT}"
