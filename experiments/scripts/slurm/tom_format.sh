...
#SBATCH --output=/hpc2hdd/home/mzhang630/data/nature/experiments/logs/tom_fmt_%j.out
...
echo "=== ToM Format Sensitivity — All 4 Models ($(date)) ==="

for MODEL_PATH in "$QWEN" "$LLAMA" "$MISTRAL" "$GEMMA"; do
    SHORT=$(basename $(dirname $(dirname "$MODEL_PATH")) | sed 's/models--[^-]*--//')
    echo ""
    echo "--- $SHORT ---"
    python3 -u src/tom_format_sensitivity.py --model_path "$MODEL_PATH" --model_short "$SHORT"
done
