...
#SBATCH --output=/hpc2hdd/home/mzhang630/data/nature/experiments/logs/emo_confuse_%j.out
...
echo "=== Emotion Confusion Test — All 4 Models ($(date)) ==="
...
for MODEL_PATH in "$QWEN" "$LLAMA" "$MISTRAL" "$GEMMA"; do
    ...
    echo ""
    echo "--- $SHORT ---"
    python3 -u src/emotion_confusion_test.py --model_path "$MODEL_PATH" --model_short "$SHORT"
done
