#!/bin/bash
#SBATCH --job-name=funcatlas_bsplit
...
#SBATCH --mem=128G
#SBATCH --time=8:00:00
#SBATCH --output=/data/user/mzhang630/logs/bsplit_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/bsplit_%A_%a.err
#SBATCH --array=0-3

# Discovery/validation split with BALANCED data (82/cat per split)
# Addresses reviewer concern: neurons found on discovery set,
# PPL measured on held-out validation set.
...
echo "Balanced Split Dissociation: ${MODEL_SHORT}"
echo "GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null)"
echo "Discovery: 82/cat, Validation: 82/cat"
