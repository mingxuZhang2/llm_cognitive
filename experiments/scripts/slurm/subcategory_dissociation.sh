#!/bin/bash
#SBATCH --job-name=funcatlas_subcat
...
#SBATCH --mem=128G
#SBATCH --time=6:00:00
#SBATCH --output=/data/user/mzhang630/logs/subcat_%A_%a.out
#SBATCH --error=/data/user/mzhang630/logs/subcat_%A_%a.err
#SBATCH --array=0-3
...
mkdir -p ${BASE}/results/subcategory /data/user/mzhang630/logs

echo "Subcategory Dissociation: ${MODEL_SHORT}"
...
python multi_function_dissociation.py \
    ...
    --stimuli "${BASE}/stimuli/stimuli_subcategory.jsonl" \
    --output_dir "${BASE}/results/subcategory" \
    --n_ablate 5000
